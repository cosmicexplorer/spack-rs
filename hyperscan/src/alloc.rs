...
/// let matches: Vec<&str> = scratch.scan_channel(&db, "asdf asdf".into(), |_| MatchResult::Continue)
...
/// let mut scratch = db.allocate_scratch()?;
/// let matches: Vec<&str> = scratch.scan_channel(&db, "hello hello".into(), |_| MatchResult::Continue)
///   .and_then(|m| async move { Ok(unsafe { m.source.as_str() }) })
///   .try_collect()
///   .await?;
